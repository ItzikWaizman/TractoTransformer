...
def plot_stats(train_stats, val_stats, num_epochs, title):
    # Unpack statistics
    train_loss, train_acc, train_acc_top_k2, train_acc_top_k1 = zip(*train_stats)
    val_loss, val_acc, val_acc_top_k2, val_acc_top_k1 = zip(*val_stats)
...
def calc_metrics(outputs, soft_labels, valid_mask, k1, k2):
    top1_pred_indices = torch.argmax(outputs, dim=-1)
    top1_label_indices = torch.argmax(soft_labels, dim=-1)
    top_k1_label_indices = torch.topk(soft_labels, k=k1, dim=-1)[1]
    top_k2_label_indices = torch.topk(soft_labels, k=k2, dim=-1)[1]
...
def fetch_hyper_params(params):
    hyper_params = {'batch_size': params.batch_size,
                    ...
                    'decay_threshold': params.improvement_threshold,
...
def fetch_metrics(train_loss, train_acc, train_acc_top_k1, val_loss, val_acc, val_acc_top_k1, train_acc_top_k2, val_acc_top_k2, params):
    metrics = {
            ...
            f'Accuracy/val_top_{params.k2}': val_acc_top_k2,
        }
    return metrics

def get_epoch_message(trainer, train_metrics, val_metrics, epoch):
    message = (f'Epoch {epoch + 1}/{trainer.params.epochs}, '
...
def load_checkpoint(trainer):
    if os.path.isfile(trainer.params.checkpoint_path):
        checkpoint = torch.load(trainer.params.checkpoint_path, map_location=trainer.device)
